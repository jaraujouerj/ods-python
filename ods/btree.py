...
class BTree(BaseSet):
    class Node(object):
        ...
        def add(self, x, ci):
            ...
            if i < 0:
                return False
            self.keys[i + 1 : len(self.keys)] = self.keys[
                i : len(self.keys) - 1
            ]
            self.children[i + 2 : len(self.children)] = self.children[
                i + 1 : len(self.children) - 1
            ]
        ...
        def remove(self, i):
            ...
            self.keys[i : len(self.keys) - 1] = self.keys[
                i + 1 : len(self.keys)
            ]
        ...
        def split(self):
            ...
            w.children[0 : len(self.children) - j - 1] = self.children[
                j + 1 : len(self.children)
            ]
    ...
    def _traverse(self, ui):
        ...
        if ui >= 0:
            ...
            while i < len(u.keys) and u.keys[i] is not None:
                for x in self._traverse(u.children[i]):
                    yield x
                yield u.keys[i]
                i += 1
            for x in self._traverse(u.children[i]):
